...
class GameState {
  ...
  const GameState({this.score = 0, this.timeLeft = 60, this.isPlaying = false, this.isGameOver = false, this.currentPackageCity, this.packageX = 0.5, this.packageY = 0.0, this.fallSpeed = 0.008, this.controlMode = 0, this.highScore = 0});

  GameState copyWith({int? score, int? timeLeft, bool? isPlaying, bool? isGameOver, String? currentPackageCity, double? packageX, double? packageY, double? fallSpeed, int? controlMode, int? highScore}) =>
    GameState(score: score ?? this.score, timeLeft: timeLeft ?? this.timeLeft, isPlaying: isPlaying ?? this.isPlaying, isGameOver: isGameOver ?? this.isGameOver, currentPackageCity: currentPackageCity ?? this.currentPackageCity, packageX: packageX ?? this.packageX, packageY: packageY ?? this.packageY, fallSpeed: fallSpeed ?? this.fallSpeed, controlMode: controlMode ?? this.controlMode, highScore: highScore ?? this.highScore);
}
...
class GameNotifier extends StateNotifier<GameState> {
  ...
  static const _cities = ['Jogja', 'Jakarta', 'Surabaya', 'Bali', 'Medan'];

  GameNotifier() : super(const GameState()) { _loadPrefs(); }

  Future<void> _loadPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    state = state.copyWith(highScore: prefs.getInt('game_high_score') ?? 0, controlMode: prefs.getInt('game_control_mode') ?? 0);
  }
  ...
  void toggleControlMode() async {
    final mode = state.controlMode == 0 ? 1 : 0;
    state = state.copyWith(controlMode: mode);
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('game_control_mode', mode);
  }

  void startGame() {
    state = GameState(isPlaying: true, controlMode: state.controlMode, highScore: state.highScore);
    _spawnPackage();
    _startCountdown();
    _startPhysics();
    if (state.controlMode == 0) _startGyroscope();
  }
  ...
  void _endGame() {
    ...
    _timer?.cancel(); _physicsTimer?.cancel(); _gyroSub?.cancel();
    _saveHighScore();
  }
  ...
  @override
  void dispose() { _timer?.cancel(); _physicsTimer?.cancel(); _gyroSub?.cancel(); super.dispose(); }
}
